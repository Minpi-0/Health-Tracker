...
const DAILY_REINFORCEMENTS = [
  { name: '死蟲式 (Dead Bug)', desc: '穩定深層核心，適合脊椎側彎日常穩定訓練。' },
  { name: '貓牛式 (Cat-Cow)', desc: '改善脊椎節律與活動度，緩解背部壓力。' },
  { name: '鳥狗式 (Bird-Dog)', desc: '強化交叉穩定力，提升身體平衡感。' },
  { name: '臀橋 (Glute Bridge)', desc: '啟動後側鏈與臀大肌，保護下腰椎。' },
  { name: '天鵝式 (Swan)', desc: '展開胸椎，強化上背部肌肉，改善體態。' },
  { name: '側板式 (Side Plank)', desc: '強化側向核心，改善身體側對線問題。' },
  { name: '孩童式 (Child\'s Pose)', desc: '深層放鬆背部與心靈，適合運動後收尾。' }
];
...
const DEFAULT_COMMON_FOODS = {
  ...
  dinner: [
    { name: '清蒸魚', pinned: true }, { name: '炒青菜', pinned: true }, { name: '豆腐', pinned: false }
  ],
  snacks: [
    { name: '綜合堅果', pinned: true }, { name: '蘋果', pinned: false }, { name: '黑咖啡', pinned: false }
  ]
};
...
const EXERCISE_LIBRARY = {
  '核心床 (Reformer)': [
    ...
    { name: '捲腹 (Crunches)', desc: '輔助深層核心收縮' }
  ],
  '凱迪拉克 (Cadillac)': [
    { name: '逐節捲動 (Roll Down)', desc: '脊椎逐節捲動，改善僵硬' },
    { name: '胸口擴展 (Chest Expansion)', desc: '開展前胸，改善圓肩' }
  ],
  '穩踏椅 (Chair)': [
    { name: '天鵝式 (Swan)', desc: '強化上背與胸椎活動' },
    { name: '推地式 (Push Down)', desc: '肩胛穩定訓練' }
  ],
  '梯桶 (Barrel)': [
    { name: '短箱系列 (Short Box)', desc: '側向穩定與軀幹拉伸' },
    { name: '側向仰臥起坐 (Side Sit-ups)', desc: '側腹線條訓練' }
  ],
  '墊上 (Mat)': [
    { name: '百次呼吸 (The Hundred)', desc: '皮拉提斯經典核心暖身' },
    { name: '死蟲式 (Dead Bug)', desc: '深層核心穩定' }
  ]
};
...
const App = () => {
  ...
  const isDietValid = useMemo(() => {
    ...
    return Number(d_water) > 0 && (d_breakfast.trim() || d_lunch.trim() || d_dinner.trim() || d_snacks.trim());
  ...
  useEffect(() => {
    ...
    const unsubPlans = onSnapshot(plansRef, (snapshot) => {
      ...
      setPlans(plansList.sort((a, b) => new Date(b.targetDate) - new Date(a.targetDate)));
      if (plansList.length > 0 && !activePlanId) setActivePlanId(plansList[0].id);
      setLoading(false);
    });
    ...
    const unsubFoods = onSnapshot(foodsRef, (docSnap) => {
      ...
      else setDoc(foodsRef, DEFAULT_COMMON_FOODS);
    });
  ...
  const syncPlanToCloud = async (updatedPlan) => {
    ...
    await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'plans', updatedPlan.id), updatedPlan);
  };

  const handleUpdateCommonFoods = async (updatedFoods) => {
    if (!user) return;
    await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'settings', 'commonFoods'), updatedFoods);
  };
  ...
  const handleCompleteRecommendation = async () => {
    ...
    const newEntry = {
      ...
      note: `補強：${todayReinforcement.name}`,
      bodyParts: ['核心'],
      apparatus: ['墊上 (Mat)'],
  ...
  const handleSaveWorkout = async () => {
    ...
    const newEntry = {
      ...
      date: w_date,
      displayDate: `${parseInt(m)}月${parseInt(d)}日`,
      note: w_note, bodyParts: w_parts, apparatus: Array.from(new Set(w_exercises.map(ex => ex.category))), exercises: w_exercises, weight: parseFloat(w_weight) || 0
    };
    ...
    closeModal();
    if (!editingId) { setShowCelebration(true); setTimeout(() => setShowCelebration(false), 2000); }
  };
  ...
  if (loading) return <div className="flex flex-col items-center justify-center min-h-screen bg-[#fbfbfe] text-[#5a6a7a] gap-4"><Loader2 className="animate-spin" size={32} /><p className="text-[12px] font-normal uppercase">找回 APP 中...</p></div>;
          ...
          {view === 'home' ? (
            <>
              ...
              <div className="grid grid-cols-2 gap-[16px]">
                <button onClick={() => setView('workout_history')} className="bg-[#e9f1fb] rounded-[24px] p-[16px] border border-[#5a6a7a]/30 flex flex-col items-center gap-4 active:scale-[0.98] transition-all group shadow-sm text-center">
                ...
                <button onClick={() => setView('diet_history')} className="bg-[#e9f1fb] rounded-[24px] p-[16px] border border-[#5a6a7a]/30 flex flex-col items-center gap-4 active:scale-[0.98] transition-all group shadow-sm text-center">
            ...
            <section className="space-y-[16px] pb-12">
              {view !== 'plan_manage' && (view === 'workout_history' ? activePlan?.workoutEntries : activePlan?.dietEntries)?.map((entry) => (
                <div key={entry.id} className="flex gap-[12px] relative">
                  ...
                  <div className="flex-1 bg-[#e9f1fb] rounded-[24px] p-[16px] border border-[#5a6a7a]/30 active:scale-[0.98] transition-all cursor-pointer shadow-sm" onClick={() => view === 'workout_history' ? handleEditWorkout(entry) : handleEditDiet(entry)}>
                      ...
                      <>
                        ...
                        <div className="space-y-2 mb-3">
                          {entry.exercises?.map((ex, i) => (
                            <div key={i} className="flex justify-between items-center bg-[#fbfbfe]/50 p-2 rounded-xl border border-[#5a6a7a]/10 shadow-sm">
              ...
              {view === 'plan_manage' && (
                <div className="space-y-[16px]">
                  <button onClick={() => setShowPlanModal(true)} className="w-full py-8 bg-[#e9f1fb] rounded-[24px] border border-[#5a6a7a]/30 text-[#040316] flex flex-col items-center gap-2 active:scale-[0.98] transition-all shadow-sm">
                    <Plus size={24} className="text-[#f9991a]"/><span className="font-medium text-[14px] uppercase tracking-widest">新增目標計畫</span>
                  </button>
                  {plans.map(plan => (
                    <div key={plan.id} className={`bg-[#e9f1fb] rounded-[24px] p-[16px] border transition-all ${activePlanId === plan.id ? 'border-[#f9991a] shadow-lg' : 'border-[#5a6a7a]/30'}`}>
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4 pt-2 border-t border-[#5a6a7a]/10">
                        <div className="flex items-center gap-2"><Dumbbell size={14} className="text-[#5a6a7a]" /><span className="text-[12px] text-[#040316]">{plan.workoutEntries?.length || 0} 筆訓練</span></div>
                        <div className="flex items-center gap-2"><Utensils size={14} className="text-[#5a6a7a]" /><span className="text-[12px] text-[#040316]">{plan.dietEntries?.length || 0} 筆飲食</span></div>
                      </div>
          ...
          <div className="absolute inset-0 z-[100] flex items-end animate-in fade-in duration-300 text-[#040316]">
            ...
            <div className="w-full bg-[#fbfbfe] rounded-t-[24px] h-[92vh] z-10 flex flex-col animate-in slide-in-from-bottom-full duration-500 shadow-2xl border-t border-[#5a6a7a]/30 relative">
              <header className="p-[16px] border-b border-[#5a6a7a]/20 flex justify-between items-center bg-[#fbfbfe]/70 backdrop-blur-[12px] z-20">
                ...
                <div className="flex bg-[#e9f1fb] p-1 rounded-xl border border-[#5a6a7a]/20 shadow-inner">
                ...
                <button onClick={closeModal} className="text-[#5a6a7a] active:scale-[0.90] transition-transform p-2"><X size={24} strokeWidth={2} /></button>
              </header>
              ...
              <div className="flex-1 overflow-y-auto p-[16px] space-y-[24px] pb-44 no-scrollbar">
                {entryType === 'workout' ? (
                  <>
                    ...
                    <div className="space-y-[12px]"><p className="text-[16px] font-medium text-[#5a6a7a] uppercase tracking-widest ml-1">訓練部位</p><div className="flex gap-3 flex-wrap">{BODY_PARTS.map(part => (<button key={part} onClick={() => setWParts(prev => prev.includes(part) ? prev.filter(p => p !== part) : [...prev, part])} className={`px-5 py-3 rounded-xl text-[14px] font-medium transition-all border ${w_parts.includes(part) ? 'bg-[#040316] text-white border-[#040316] shadow-md' : 'bg-[#fbfbfe] text-[#5a6a7a] border-[#5a6a7a]/30'}`}>{part}</button>))}</div></div>
                    <div className="space-y-[12px]"><p className="text-[16px] font-medium text-[#5a6a7a] uppercase tracking-widest ml-1">器械與動作選擇</p><div className="bg-[#e9f1fb]/50 rounded-[24px] p-4 border border-[#5a6a7a]/20 space-y-5 shadow-inner"><div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">{APPARATUS_TYPES.map(type => (<button key={type} onClick={() => setWApparatus(type)} className={`px-4 py-2.5 rounded-xl text-[14px] font-medium whitespace-nowrap active:scale-[0.98] transition-all border ${w_apparatus === type ? 'bg-[#040316] text-white border-[#040316] shadow-md' : 'bg-[#fbfbfe] text-[#5a6a7a] border-[#5a6a7a]/20'}`}>{type}</button>))}</div><div className="grid grid-cols-1 gap-3">{EXERCISE_LIBRARY[w_apparatus]?.map(ex => { const idx = w_exercises.findIndex(i => i.name === ex.name && i.category === w_apparatus); const active = idx !== -1; return (<div key={ex.name} className={`flex flex-col rounded-[20px] border transition-all p-3 shadow-sm ${active ? 'border-[#f9991a] bg-[#fbfbfe]' : 'border-[#5a6a7a]/10 bg-[#fbfbfe]/70'}`}><div className="flex items-center gap-4 p-1 w-full"><button onClick={() => active ? setWExercises(w_exercises.filter((_, i) => i !== idx)) : setWExercises([...w_exercises, { name: ex.name, sets: '', category: w_apparatus }])} className="flex-1 flex items-center gap-3 text-left"><div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${active ? 'bg-[#f9991a] text-white' : 'bg-[#e9f1fb] text-[#5a6a7a] shadow-inner'}`}>{active ? <Check size={18} /> : <Plus size={18} />}</div><div><p className={`text-[14px] font-medium text-[#040316]`}>{ex.name}</p></div></button><a href={`https://www.youtube.com/results?search_query=pilates+${ex.name.split(' (')[0]}`} target="_blank" rel="noreferrer" className="p-3 bg-[#e9f1fb] rounded-xl text-red-500 shadow-sm active:scale-90" onClick={(e) => e.stopPropagation()}><Youtube size={18} /></a></div>{active && (<div className="px-1 pb-1 pt-2 animate-in slide-in-from-top-2"><div className="flex items-center gap-2 bg-[#e9f1fb] px-4 py-3 rounded-xl border border-[#5a6a7a]/20 shadow-inner"><Hash size={12} className="text-[#5a6a7a]" /><input type="text" placeholder="設定組數 (例如: 15x3)" value={w_exercises[idx].sets} onChange={(e) => setWExercises(w_exercises.map((item, i) => i === idx ? { ...item, sets: e.target.value } : item))} className="bg-transparent text-[14px] font-normal outline-none w-full text-[#040316]" /></div></div>)}</div>);})}</div></div></div>
                    <div className="space-y-[12px]"><p className="text-[16px] font-medium text-[#5a6a7a] uppercase tracking-widest ml-1">訓練筆記</p><textarea value={w_note} onChange={(e) => setWNote(e.target.value)} className="w-full h-32 bg-[#e9f1fb] rounded-[16px] p-[16px] text-[14px] font-normal border border-[#5a6a7a]/30 outline-none focus:border-[#5a6a7a] shadow-inner resize-none" placeholder="分享訓練感受..." /></div>
                  </>
                ) : (
                  <>
                    <div className="bg-[#e9f1fb] p-[16px] rounded-[16px] border border-[#5a6a7a]/30 text-center space-y-[12px]"><p className="text-[16px] font-medium text-[#5a6a7a] uppercase tracking-widest">飲食日期</p><input type="date" value={d_date} onChange={(e) => setDDate(e.target.value)} className="w-full bg-transparent font-semibold text-[18px] border-none outline-none text-center text-[#f9991a]" /></div>
                    <div className="space-y-[12px]"><div className="bg-[#e9f1fb] p-[16px] rounded-[24px] flex items-center justify-between border border-[#5a6a7a]/50 shadow-inner"><div className="flex items-center gap-4"><div className="text-[#f9991a]"><Droplets size={24} /></div><div className="flex items-center"><input type="number" inputMode="numeric" pattern="[0-9]*" value={d_water} onChange={(e) => setDWater(e.target.value)} className="w-20 bg-transparent text-[28px] font-semibold text-[#040316] tracking-tighter outline-none" /><span className="text-[14px] font-normal text-[#5a6a7a] ml-1">cc</span></div></div><div className="flex items-center gap-3"><button onClick={() => setDWater(Math.max(0, Number(d_water) - 250))} className="w-12 h-12 bg-[#fbfbfe] rounded-full flex items-center justify-center border border-[#5a6a7a]/30 text-[#040316] active:scale-[0.90] shadow-sm text-xl">-</button><button onClick={() => setDWater(Number(d_water) + 250)} className="w-12 h-12 bg-[#f9991a] rounded-full flex items-center justify-center text-white active:scale-[0.90] shadow-md text-xl shadow-orange-200">+</button></div></div></div>
                    <div className="space-y-[24px]">{[{ label: '早餐', val: d_breakfast, set: setDBreakfast, icon: <Coffee size={18}/>, key: 'breakfast' }, { label: '午餐', val: d_lunch, set: setDLunch, icon: <Sun size={18}/>, key: 'lunch' }, { label: '晚餐', val: d_dinner, set: setDDinner, icon: <Moon size={18}/>, key: 'dinner' }, { label: '點心/其他', val: d_snacks, set: setDSnacks, icon: <Cookie size={18}/>, key: 'snacks' }].map(meal => {
                      const sortedFoods = [...(commonFoods[meal.key] || [])].sort((a, b) => b.pinned - a.pinned);
                      const isManaging = managingMealType === meal.key;
                      return (<div key={meal.label} className="space-y-[12px]"><div className="flex justify-between items-center px-1"><div className="flex items-center gap-2 text-[#5a6a7a]"><span>{meal.icon}</span><p className="text-[16px] font-medium uppercase tracking-widest">{meal.label}</p></div><button onClick={() => setManagingMealType(isManaging ? null : meal.key)} className={`p-2 rounded-lg transition-all shadow-sm ${isManaging ? 'bg-[#f9991a] text-white shadow-md' : 'text-[#5a6a7a] bg-gray-100'}`}><Settings size={18} /></button></div>{isManaging ? (<div className="bg-[#e9f1fb] rounded-[24px] p-4 border border-[#f9991a]/30 space-y-4 animate-in slide-in-from-top-2 shadow-inner"><div className="flex gap-2"><input value={newFoodInput} onChange={(e) => setNewFoodInput(e.target.value)} placeholder="輸入新食物名稱..." className="flex-1 bg-[#fbfbfe] rounded-xl px-4 py-2 text-[14px] outline-none border border-[#5a6a7a]/20 shadow-sm" /><button onClick={() => handleAddFood(meal.key)} className="bg-[#f9991a] text-white p-2 rounded-xl active:scale-95 shadow-md"><Plus size={20} /></button></div><div className="space-y-2 max-h-[200px] overflow-y-auto no-scrollbar">{commonFoods[meal.key].map((food, idx) => (<div key={idx} className="flex items-center justify-between bg-[#fbfbfe] p-3 rounded-xl border border-[#5a6a7a]/10 shadow-sm"><p className="text-[14px] font-medium">{food.name}</p><div className="flex items-center gap-2"><button onClick={() => handleTogglePin(meal.key, idx)} className={`p-1.5 rounded-lg shadow-sm ${food.pinned ? 'bg-[#f9991a]/10 text-[#f9991a]' : 'bg-gray-50 text-[#5a6a7a]'}`}>{food.pinned ? <Pin size={16} fill="currentColor" /> : <PinOff size={16} />}</button><button onClick={() => handleRemoveFood(meal.key, idx)} className="p-1.5 text-red-400 bg-red-50 rounded-lg shadow-sm"><Trash2 size={16} /></button></div></div>))}</div></div>) : (<div className="bg-[#fbfbfe] rounded-[24px] p-[16px] border border-[#5a6a7a]/30 flex flex-col gap-3 focus-within:border-[#5a6a7a] transition-all shadow-sm"><textarea value={meal.val} onChange={(e) => meal.set(e.target.value)} placeholder="點擊下方常用或手寫紀錄..." className="w-full bg-transparent border-none outline-none text-[14px] font-normal resize-none min-h-[60px]" /><div className="flex gap-2 overflow-x-auto no-scrollbar pt-2 border-t border-[#5a6a7a]/10">{sortedFoods.map((food, idx) => (<button key={idx} onClick={() => handleQuickAddFood(meal.set, meal.val, food.name)} className={`px-3 py-1.5 rounded-lg text-[12px] whitespace-nowrap active:scale-95 border flex items-center gap-1 shadow-sm ${food.pinned ? 'bg-[#f9991a]/10 text-[#f9991a] border-[#f9991a]/30' : 'bg-[#e9f1fb] text-[#5a6a7a] border-[#5a6a7a]/10'}`}>{food.pinned && <Pin size={10} fill="currentColor" />}{food.name}</button>))}</div></div>)}</div>);
                    })}
                      <div className="space-y-[12px]"><p className="text-[16px] font-medium text-[#5a6a7a] uppercase tracking-widest ml-1">飲食標籤</p><div className="flex gap-3 flex-wrap">{DIET_TAGS.map(tag => <button key={tag} onClick={() => setDTags(prev => prev.includes(tag) ? prev.filter(t => t !== tag) : [...prev, tag])} className={`px-4 py-3 rounded-xl text-[12px] font-medium transition-all border shadow-sm ${d_tags.includes(tag) ? 'bg-[#040316] text-white border-[#040316]' : 'bg-[#fbfbfe] text-[#5a6a7a] border-[#5a6a7a]/30'}`}>{tag}</button>)}</div></div>
                    </div>
                  </>
                )}
                {editingId && <button onClick={handleDelete} className="w-full py-4 text-red-600 font-medium text-[14px] bg-[#fbfbfe] border border-red-100 rounded-xl active:scale-[0.98] transition-all mb-8 shadow-sm shadow-red-50">刪除這筆紀錄</button>}
              </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-[16px] bg-[#fbfbfe]/70 backdrop-blur-[12px] border-t border-[#5a6a7a]/20 z-30 shadow-[0_-10px_30px_rgba(0,0,0,0.03)]">
                ...
                <button onClick={entryType === 'workout' ? handleSaveWorkout : handleSaveDiet} disabled={!activePlan || (entryType === 'diet' ? !isDietValid : !isWorkoutValid)} className={`w-full py-4 rounded-[16px] text-white font-semibold text-[16px] shadow-lg active:scale-[0.98] transition-all ${entryType === 'workout' ? 'bg-[#f9991a] shadow-orange-100' : 'bg-[#040316] shadow-gray-200'} disabled:opacity-30 disabled:active:scale-100`}>{editingId ? '確認並修改' : `儲存${entryType === 'workout' ? '訓練' : '飲食'}紀錄`}</button>
              </div>
            </div>
          </div>
        )}
        ...
        {showPlanModal && (
          <div className="absolute inset-0 z-[110] flex items-center justify-center p-4 animate-in fade-in duration-300">
            ...
            <div className="w-full max-w-sm bg-[#fbfbfe] rounded-[24px] p-6 z-10 shadow-2xl border border-[#5a6a7a]/20 text-[#040316]">
              <h3 className="text-[20px] font-medium mb-4 text-center">新增目標計畫</h3>
              <div className="space-y-4">
                ...
                <div className="space-y-1"><p className="text-[16px] font-medium text-[#5a6a7a]">目標日期</p><input type="date" value={newPlanDate} onChange={(e) => setNewPlanDate(e.target.value)} className="w-full bg-[#e9f1fb] rounded-xl p-4 text-[14px] font-medium outline-none shadow-inner" /></div>
              ...
              <div className="flex gap-3 mt-8">
                <button onClick={() => setShowPlanModal(false)} className="flex-1 py-3 text-[#5a6a7a] font-normal text-[14px] active:scale-95 transition-all">取消</button>
                <button onClick={handleCreatePlan} className="flex-1 py-3 bg-[#f9991a] text-white rounded-xl font-semibold text-[14px] shadow-lg active:scale-[0.98] transition-all shadow-orange-100">啟動計畫</button>
              </div>
            </div>
          </div>
        )}
